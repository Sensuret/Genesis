...
import { BalanceSpeakButton } from "@/components/ui/balance-speak-button";
...
export default function ReportsPage() {
  ...
  return (
    <div ref={screenshotRef} className="space-y-6">
        ...
        <PageHeader
          ...
          actions={
            <ScreenshotButton
              ...
              filename={`reports-${tab.toLowerCase().replace(/\s+/g, "-")}`}
            />
          }
        />
      ...
      {tab === "Detailed" && (
        <>
          <ReportsDetailed trades={filtered} fmt={fmt} />
      ...
      {tab === "Deep Stats" && (
        <DeepStats
          trades={filtered}
          fmt={fmt}
          fileBalance={fileAggregates.balance}
...
function Overview({
  ...
  const equityData = useMemo(() => dailyEquityCurve(trades, 0), [trades]);
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="p-5">
          ...
          <div className="text-2xl font-semibold tracking-tight text-fg">
            {fileAggregates.balance == null ? "—" : fmt(fileAggregates.balance)}
          </div>
        </Card>
        <Stat
  ...
  return (
    <div className="space-y-4">
      <div className="mt-6 flex items-baseline justify-between gap-3">
        ...
        <span className="text-xs text-fg-muted">Setups, emotions, sessions, days, pairs &amp; correlations</span>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
        ...
        <Card>
          <CardHeader><CardTitle>By emotion</CardTitle></CardHeader>
          <CardBody>
            {byEmotion.length === 0 ? (
              <div className="text-sm text-fg-muted">Tag emotions on trades to see performance by emotion.</div>
            ) : (
              <PerfBar
      ...
      <Card>
        ...
        <CardBody>
          <p className="mb-3 text-xs text-fg-muted">
            Planned R:R from entry, stop-loss and take-profit; when TP is missing we use exit vs stop as realized R.
          </p>
