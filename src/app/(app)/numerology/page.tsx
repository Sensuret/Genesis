...
import { BirthPlaceFields } from "@/components/numerology/birth-place-fields";
...
import { approximateMoonSign, approximateAscendantHint } from "@/lib/astrology/moon-sign";
...
function timeOfBirthFromData(data: unknown): string {
  if (!data || typeof data !== "object") return "";
  const v = (data as { timeOfBirth?: unknown }).timeOfBirth;
  return typeof v === "string" ? v : "";
}

function tobFormatFromData(data: unknown): "12" | "24" {
  if (!data || typeof data !== "object") return "24";
  return (data as { tobFormat?: unknown }).tobFormat === "12" ? "12" : "24";
}

function birthCountryFromData(data: unknown): string {
  if (!data || typeof data !== "object") return "";
  const v = (data as { birthCountry?: unknown }).birthCountry;
  return typeof v === "string" ? v : "";
}

function birthCityFromData(data: unknown): string {
  if (!data || typeof data !== "object") return "";
  const v = (data as { birthCity?: unknown }).birthCity;
  return typeof v === "string" ? v : "";
}
...
function MyProfile({
  ...
  const [tobFormat, setTobFormat] = useState<"12" | "24">(() => tobFormatFromData(profile?.data));
  const [birthCountry, setBirthCountry] = useState(() => birthCountryFromData(profile?.data));
  const [birthCity, setBirthCity] = useState(() => birthCityFromData(profile?.data));
          ...
          <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_minmax(0,18rem)] md:items-start">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <div className="w-full min-w-[280px] flex-[1_1_100%]">
                  <BirthPlaceFields
                    optional
      ...
      {snapshot && sign && (
        <>
          ...
          <Card>
            ...
            <CardBody>
              <NatalChartWheel dob={dob} fullName={name} timeOfBirth={tob || undefined} />
            </CardBody>
          </Card>
...
function Others({
  ...
  async function add() {
    ...
    const dataPayload = {
      ...
      birthCountry: form.birthCountry.trim() || undefined,
  ...
  async function update(id: string, patch: NumerologyOtherUpdate) {
    ...
    const nextTob = patch.timeOfBirth ?? timeOfBirthFromData(target.data);
    const nextTobFormat = patch.tobFormat ?? tobFormatFromData(target.data);
    const nextBirthCountry = patch.birthCountry ?? birthCountryFromData(target.data);
    const nextBirthCity = patch.birthCity ?? birthCityFromData(target.data);
    ...
    const dataPayload = {
      ...
      nicknames: cleanNicks,
      timeOfBirth: nextTob || undefined,
      tobFormat: nextTob ? nextTobFormat : undefined,
      birthCountry: nextBirthCountry.trim() || undefined,
      birthCity: nextBirthCity.trim() || undefined,
    };
            ...
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <div>
                <BirthPlaceFields
                  ...
                  city={form.birthCity}
                  onCountryChange={(birthCountry) => setForm({ ...form, birthCountry })}
                  onCityChange={(birthCity) => setForm({ ...form, birthCity })}
...
function OtherDetailModalContent({
  ...
  const [draft, setDraft] = useState({
    ...
    tob: timeOfBirthFromData(row.data),
    tobFormat: tobFormatFromData(row.data),
    birthCountry: birthCountryFromData(row.data),
    birthCity: birthCityFromData(row.data),
  });
  ...
  async function saveEdit() {
    ...
    const patch: NumerologyOtherUpdate = {
      ...
      timeOfBirth: draft.tob,
      tobFormat: draft.tobFormat,
      ...
      >
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <Input
                  type="time"
                  value={draft.tob}
                  onChange={(e) => setDraft({ ...draft, tob: e.target.value })}
                />
              </div>
              <div>
                <Label>Time format</Label>
                <Select
                  value={draft.tobFormat}
                  onChange={(e) =>
                    setDraft({ ...draft, tobFormat: e.target.value as "12" | "24" })
                  }
                >
                  <option value="24">24hr</option>
                  <option value="12">12hr</option>
              ...
              <div className="md:col-span-2">
                <BirthPlaceFields
                  ...
                  onCountryChange={(birthCountry) => setDraft({ ...draft, birthCountry })}
                  onCityChange={(birthCity) => setDraft({ ...draft, birthCity })}
                />
              </div>
            </div>
            <NicknameInputs
        ...
        <div className="mt-4 space-y-4">
          <BirthChartSnapshot
            ...
            sign={sign}
            snapshot={snap}
            timeOfBirth={timeOfBirthFromData(row.data)}
            birthCountry={birthCountryFromData(row.data)}
            birthCity={birthCityFromData(row.data)}
          />
          <Card>
            ...
            <CardBody>
              <NatalChartWheel
                dob={row.dob}
                fullName={row.full_name}
                timeOfBirth={timeOfBirthFromData(row.data) || undefined}
              />
            </CardBody>
          </Card>
          <GemstonesCard
...
function SectionBanner({
  ...
  const palette =
    accent === "brand"
      ? {
          ring: "border-brand-500/50",
          surface: "bg-bg-elevated",
          chipText: "text-brand-600 dark:text-brand-200",
          chipBorder: "border-brand-500/45",
          chipBg: "bg-brand-500/15",
          accentBar: "bg-gradient-to-b from-cyan-400 via-brand-500 to-violet-600"
        }
      : {
          ring: "border-warn/50",
          surface: "bg-bg-elevated",
          chipText: "text-amber-700 dark:text-warn",
          chipBorder: "border-warn/45",
          chipBg: "bg-warn/15",
          accentBar: "bg-gradient-to-b from-amber-400 via-warn to-amber-600"
        };
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border ${palette.ring} ${palette.surface} px-5 py-5 shadow-card sm:px-6 sm:py-6`}
    >
      <div className={`absolute inset-y-3 left-0 w-1 rounded-r-full ${palette.accentBar}`} aria-hidden />
      <div className="relative flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1 pl-3">
          <div
            className={`text-[11px] font-bold uppercase tracking-[0.2em] ${palette.chipText}`}
          >
            {eyebrow}
          </div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-fg">{body}</p>
        </div>
        <div
  ...
  return (
    <Card>
      <CardBody className="space-y-3">
        ...
        <div className="rounded-lg border border-line bg-bg-soft p-3 text-xs leading-relaxed text-fg">
...
function BirthChartSnapshot({
  ...
  const moonSign = timeOfBirth ? approximateMoonSign(dob, timeOfBirth) : null;
  const ascHint =
    timeOfBirth && sign.sign
      ? approximateAscendantHint(dob, timeOfBirth, sign.sign as WesternSign)
      : null;
  const birthPlace =
    [birthCity?.trim(), birthCountry?.trim()].filter(Boolean).join(", ") || null;
  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-3 text-sm">
        <p className="text-xs text-fg-muted">
          Placements derived from your date of birth
          {timeOfBirth ? ", birth time" : ""}
          {birthPlace ? `, and birthplace (${birthPlace})` : ""}.
          {timeOfBirth
            ? " Moon and approximate rising are included; exact house cusps need precise coordinates."
            : " Add optional birth time for Moon, rising estimate, and sharper horoscope tie-ins."}
        </p>

        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          <ChartCell label="Sun (Western)" value={`${sign.sign} ${sign.symbol}`} note={`${sign.element} · ${sign.modality} · ruled by ${sign.ruler}`} />
          {moonSign ? (
            <ChartCell label="Moon" value={moonSign} note="Approximate sign from birth time" />
          ) : (
            <ChartCell label="Moon" value="—" note="Add birth time to estimate" />
          )}
          {ascHint ? (
            <ChartCell label="Ascendant (Rising)" value={ascHint.split(" (")[0]} note={ascHint.includes("approx") ? ascHint : undefined} />
          ) : (
            <ChartCell label="Ascendant (Rising)" value="—" note="Add birth time (+ city for exact)" />
          )}
          {birthPlace ? <ChartCell label="Birthplace" value={birthPlace} /> : null}
        ...
        {!timeOfBirth && (
          <div className="rounded-xl border border-brand-400/25 bg-brand-500/5 p-3 text-xs text-fg-muted">
            <span className="font-medium text-brand-200">Optional birth time:</span>{" "}
            Add time (and city/country) on your profile for Moon sign, rising estimate, and
            personalized horoscope links on the natal wheel.
          </div>
        )}
      </CardBody>
    </Card>
