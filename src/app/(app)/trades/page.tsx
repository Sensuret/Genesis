...
export default function TradesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return real.filter((t) => {
      ...
      if (!q) return true;
      const emotionBlob = (t.emotions?.join(" ") ?? t.mistake_tag ?? "");
      const blob = `${t.pair ?? ""} ${t.setup_tag ?? ""} ${emotionBlob} ${t.notes ?? ""}`.toLowerCase();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Input
          placeholder="Search pair, setup, emotion, notes…"
          value={q}
      ...
      {loading ? (
        ...
        <Card>
          <TradeLogTable trades={filtered} balanceFallback={balanceFallback} editableTags />
        </Card>
      )}
    </div>
  );
