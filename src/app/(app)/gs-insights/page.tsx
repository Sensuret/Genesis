  ...
  if (rrs.length >= 10) {
    if (avgRR < 1) {
      ...
      out.push({
        ...
        detail: `Avg planned setup is 1:${avgRR.toFixed(2)} risk-to-reward.`
      });
    }
  }
  ...
  if (loading) {
    return (
      <div ref={screenshotRef} className="space-y-4">
        <div className="text-sm text-fg-muted">Loading insights…</div>
      </div>
    );
  }
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          <YAxis
            ...
            width={dataKey === "profitFactor" ? 56 : 44}
            tickMargin={4}
          />
          <Tooltip
            {...chartTooltipProps}
