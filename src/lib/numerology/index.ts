...
import { filteredAdvancedInsights } from "./zodiac-insight-filters";
...
export function advancedInsights(snapshot: NumerologySnapshot) {
  const base = INSIGHTS[snapshot.lifePath] ?? INSIGHTS[1];
  return filteredAdvancedInsights(base, snapshot);
}
