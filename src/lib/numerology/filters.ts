...
function genderFromUnknown(data: unknown): CombinedProfile["gender"] {
...
export function buildCombinedProfiles(
  ...
  if (profile) {
    out.push({
      ...
      relationship: "",
      dob: profile.dob,
      gender: genderFromUnknown(profile.data),
  ...
  for (const r of sortedOthers) {
    ...
    out.push({
      ...
      dob: r.dob,
      gender: genderFromUnknown(r.data),
...
export type NumFilters = {
  ...
  dobFrom?: string;
  dobTo?: string;
  gender?: Array<"male" | "female" | "prefer_not_to_say">;
};
...
export function applyFilters(
  ...
  return list.filter((p) => {
    ...
    if (filters.gender?.length && (!p.gender || !filters.gender.includes(p.gender))) {
      return false;
    }
    return true;
  });
