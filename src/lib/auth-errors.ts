...
export function friendlyAuthMessage(
  ...
  if (
    ...
    return "Cannot reach Supabase. Open Genesis-main-current/.env.local (URL + anon/publishable key), save it, stop the dev server (Ctrl+C), run `npm run dev` again, and use the exact localhost URL shown in the terminal (often :3000 or :3001).";
  }
