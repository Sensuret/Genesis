 ...
 */
/**
 * Realized R at close when SL and exit are known (fallback when TP was not set).
 */
export function achievedRR(t: TradeRow): number | null {
  if (t.entry == null || t.stop_loss == null || t.stop_loss === 0) return null;
  if (t.exit_price == null || t.exit_price === 0) return null;
...
export function plannedOrAchievedRR(t: TradeRow): number | null {
  return realisedRR(t) ?? achievedRR(t);
}
...
export function applyPlaybookFilter(
  trades: TradeRow[],
  playbookId: string | null,
  linkedAccountIds: string[] = []
): TradeRow[] {
  if (!playbookId) return trades;
  const linked = linkedAccountIds.length ? new Set(linkedAccountIds) : null;
  return trades.filter((t) => {
    if (t.playbook_id === playbookId) return true;
    if (linked && t.file_id && linked.has(t.file_id)) return true;
    return false;
  });
}
...
export function applyAllFilters(trades: TradeRow[], filters: AppFilters): TradeRow[] {
  ...
  return applyPlaybookFilter(acct, filters.playbookId, filters.playbookLinkedAccounts);
}
...
export function sortByDate(trades: TradeRow[]): TradeRow[] {
  return [...trades].sort((a, b) => {
    ...
    const ac = a.close_time ? new Date(a.close_time).getTime() : 0;
    const bc = b.close_time ? new Date(b.close_time).getTime() : 0;
...
/** Cumulative P&L by calendar day — stable when live trades reorder intraday. */
export function dailyEquityCurve(trades: TradeRow[], start = 0): EquityPoint[] {
  const daily = dailyPnl(trades);
  let equity = start;
  return daily.map((d) => {
    equity += d.pnl;
    return { date: d.date, pnl: d.pnl, equity };
  });
}
...
export function dailyAccountBalanceCurve(
  ...
  for (const t of sorted) {
    if (!t.trade_date || t.account_balance == null || !Number.isFinite(t.account_balance)) continue;
    const day = t.trade_date.slice(0, 10);
    byDay.set(day, t.account_balance);
  }
  if (byDay.size > 0) {
    return Array.from(byDay.entries())
      .sort(([a], [b]) => (a < b ? -1 : 1))
      .map(([date, equity]) => ({ date, pnl: 0, equity }));
  }

  if (!sorted.length) return [];

  const total = totalPnl(sorted);
  let start = options?.startingBalance ?? null;
  if (start == null && options?.fileBalance != null && Number.isFinite(options.fileBalance)) {
    start = options.fileBalance - total;
  }
  if (start == null) {
    const withBal = sorted
  ...
  if (
    curve.length &&
    options?.fileBalance != null &&
    Number.isFinite(options.fileBalance)
  ) {
    const last = curve[curve.length - 1]!;
    const lastDay = sorted[sorted.length - 1]?.trade_date?.slice(0, 10) ?? last.date;
    if (Math.abs(last.equity - options.fileBalance) > 0.01) {
      curve.push({ date: lastDay, pnl: 0, equity: options.fileBalance });
    }
  }
  return curve;
}

/** Break down performance by emotion tags (falls back to legacy mistake_tag). */
export function performanceByEmotions(trades: TradeRow[]) {
  const buckets: Record<string, TradeRow[]> = {};
  for (const t of trades) {
    const tags: string[] = [];
    if (t.emotions?.length) tags.push(...t.emotions.filter(Boolean));
    else if (t.mistake_tag?.trim()) tags.push(t.mistake_tag.trim());
