...
import { BrandLockup, ResolutionWatermark } from "@/components/logo";
...
const ROTATE3D_STORAGE_KEY = "gs.resolutions.rotate3d.v1";
...
export function ResolutionsTab({
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.localStorage.getItem(ROTATE3D_STORAGE_KEY);
      if (raw === "1") setRotate3d(true);
    } catch {
      // ignore
    }
  }, []);

  function setRotate3dPersist(next: boolean) {
    setRotate3d(next);
    if (typeof window !== "undefined") {
      try {
        window.localStorage.setItem(ROTATE3D_STORAGE_KEY, next ? "1" : "0");
      } catch {
        // ignore
      }
    }
      ...
      <div className="flex items-center gap-3">
        ...
        {sub !== "create" && (
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-fg-muted">3D rotate</span>
            <ToggleSwitch
              checked={rotate3d}
              onChange={setRotate3dPersist}
              label="Toggle 3D card rotation"
            />
          </div>
        )}
      </div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {resolutions.map((r) => (
        <div key={r.id} className="group relative">
          <div className={rotate3d ? "resolution-flip-scene" : undefined}>
            <div
              className={rotate3d ? "resolution-flip-inner" : undefined}
              style={rotate3d ? { minHeight: "12rem" } : undefined}
            >
              <div className={rotate3d ? "resolution-flip-face" : undefined}>
                <div
                  role="button"
                  tabIndex={0}
                  onClick={(e) => {
              ...
              {rotate3d && (
                <div className="resolution-flip-back resolution-flip-face">
                  <ResolutionWatermark className="!bottom-auto !inset-x-0 opacity-[0.12]" />
                  <BrandLockup wordmarkSize="md" markClassName="h-14" className="wordmark-solid relative z-[1] text-white" />
                </div>
              )}
            </div>
          </div>
          ...
          <button
            ...
            onClick={(e) => {
              ...
              const ok = window.confirm(
                "Delete this resolution permanently? You won't be able to view it again."
              );
              if (ok) onDelete(r.id);
            }}
