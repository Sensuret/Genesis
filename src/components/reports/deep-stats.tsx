...
import { dailyAccountBalanceCurve } from "@/lib/analytics";
...
function VisualSummary({
  ...
  return (
    <Card>
      <CardBody>
        <div className="grid gap-6 lg:grid-cols-[280px_1fr_1fr_minmax(200px,280px)]">
          ...
          <div className="flex flex-col justify-center">
            <div className="mb-2 text-xs font-medium text-fg-subtle">Account balance</div>
            <EquityCurveChart
              data={balanceCurve}
              height="h-44"
              variant="purple"
              emptyTitle={trades.length ? "No balance history" : "No trades yet"}
              emptyDescription={
                trades.length
                  ? "Add account balance on imports or log trades with P&L to build the curve."
                  : "Import or log trades to see account balance over time."
              }
            />
          </div>
        </div>
