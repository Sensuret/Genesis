...
export function LunarPerformance({ trades, fmt: ccyFmt }: { trades: TradeRow[]; fmt: CcyFmt }) {
  ...
  const currentPhase = useMemo(() => moonPhase(new Date()).phase, []);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {best && best.trades > 0 && (
          <CycleHighlight
            label="Best Lunar Cycle"
            phase={best}
            tone="success"
            ccyFmt={ccyFmt}
            isCurrent={currentPhase === best.phase}
          />
        )}
        {worst && worst.trades > 0 && (
          <CycleHighlight
            label="Worst Lunar Cycle"
            phase={worst}
            tone="danger"
            ccyFmt={ccyFmt}
            isCurrent={currentPhase === worst.phase}
          />
        )}
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {phases.map((p) => (
          <PhaseCard
            ...
            isCurrent={currentPhase === p.phase}
            ccyFmt={ccyFmt}
          />
        ))}
      </div>
              ...
              <tbody>
                {phases.map((p) => (
                  <tr
                    key={p.phase}
                    className={cn(
                      "border-b border-line/50 last:border-0 hover:bg-bg-soft/50",
                      currentPhase === p.phase && "ring-2 ring-inset ring-brand-500/40"
                    )}
                  >
...
function CosmicDeepStatsTab({ zodiacPerf, retroPerf, transitPerf, heatmap, cosmicWindows, insights, ccyFmt }: {
  ...
  const currentZodiac = useMemo(() => zodiacSeasonForDate(new Date()), []);
  const currentPhase = useMemo(() => moonPhase(new Date()).phase, []);
  const currentTransit = `Sun in ${currentZodiac}`;
  const mercuryRetrogradeNow = useMemo(() => isInMercuryRetrograde(new Date()), []);

  const isCurrentCosmicWindow = (w: CosmicWindow) =>
    w.label === currentPhase ||
    w.label === `${currentZodiac} season` ||
    (w.label === "Mercury Retrograde" && mercuryRetrogradeNow);
              ...
              <tbody>
                {zodiacPerf.map((z) => (
                  <tr
                    ...
                    className={cn(
                      "border-b border-line/50 last:border-0 hover:bg-bg-soft/50",
                      currentZodiac === z.sign && "ring-2 ring-inset ring-brand-500/40"
                    )}
                  >
                    <td className="px-3 py-2 font-medium">
            ...
            <BarChart data={zodiacChart} margin={{ top: 4, right: 4, bottom: 4, left: 0 }}>
              ...
              <Bar dataKey="netPnl" name="Net P&L" radius={[3, 3, 0, 0]}>
                {zodiacChart.map((entry, i) => (
                  <Cell key={`zodiac-${i}`} fill={entry.netPnl >= 0 ? "#22c55e" : "#ef4444"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            <BarChart data={transitChart} margin={{ top: 4, right: 4, bottom: 4, left: 0 }}>
              ...
              <Bar dataKey="winRate" name="Win Rate %" radius={[3, 3, 0, 0]}>
                {transitChart.map((entry, i) => (
                  <Cell key={`transit-${i}`} fill={entry.winRate >= 50 ? "#22c55e" : "#ef4444"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
                ...
                <tbody>
                  {cosmicWindows.slice(0, 10).map((w, i) => (
                    <tr
                      ...
                      className={cn(
                        "border-b border-line/50 last:border-0 hover:bg-bg-soft/50",
                        isCurrentCosmicWindow(w) && "ring-2 ring-inset ring-brand-500/40"
                      )}
                    >
                      <td className="px-3 py-2 font-medium">
              ...
              <tbody>
                {transitPerf.map((t) => (
                  <tr
                    ...
                    className={cn(
                      "border-b border-line/50 last:border-0 hover:bg-bg-soft/50",
                      currentTransit === t.transit && "ring-2 ring-inset ring-brand-500/40"
                    )}
                  >
                    <td className="px-3 py-2 font-medium">
...
function CycleHighlight({ label, phase, tone, ccyFmt, isCurrent }: {
  label: string;
...
function PhaseCard({ phase, isBest, isWorst, isCurrent, ccyFmt }: {
  phase: LunarPhasePerformance;
  isBest: boolean;
  isWorst: boolean;
  isCurrent?: boolean;
