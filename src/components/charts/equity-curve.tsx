...
export function EquityCurveChart({
  ...
  emptyDescription = "Equity curve will populate as you log or import trades."
}: {
  data: EquityPoint[];
  ...
  const fillId = `equity-fill-${uid}`;
  const strokeId = `equity-stroke-${uid}`;
  ...
  if (!data.length) return <Empty title={emptyTitle} description={emptyDescription} />;

  return (
    <div className={`${height} w-full`}>
      <ResponsiveContainer>
        <AreaChart data={converted} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
          <defs>
            {variant === "purple" ? (
              <>
