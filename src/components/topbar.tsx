...
import { readRules } from "@/lib/playbooks";
...
export function TopBar() {
  ...
  useEffect(() => {
    ...
    // Unique channel name per mount so React Strict Mode never reuses a subscribed channel.
    const channelId = `topbar-playbooks-${Math.random().toString(36).slice(2)}`;
    ...
    async function refreshPlaybooks() {
      ...
      if (!cancelled) setPlaybooks((data ?? []) as PlaybookRow[]);
    }
  ...
  return (
    <header data-screenshot-ignore="true" className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-line bg-bg/85 px-6 backdrop-blur">
      ...
      <PlaybookPicker
        ...
        onChange={(v) => {
          const pb = v ? playbooks.find((p) => p.id === v) : null;
          const linked = pb ? readRules(pb.rules).linkedAccounts ?? [] : [];
          setFilters({
            playbookId: v,
            playbookLinkedAccounts: linked,
            // When a playbook defines linked accounts, auto-scope account filter.
            accountIds: linked.length ? linked : filters.accountIds
          });
        }}
      />
