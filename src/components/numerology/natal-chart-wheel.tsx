...
import { approximateMoonSign } from "@/lib/astrology/moon-sign";
...
function computePlacements(dob: string, timeOfBirth?: string): PlanetPlacement[] {
  ...
  if (timeOfBirth) {
    const moonSign = approximateMoonSign(dob, timeOfBirth);
    if (moonSign) {
      placements.push({
        name: "Moon",
        glyph: PLANET_GLYPHS.Moon,
        sign: moonSign,
        longitude: signToLongitude(moonSign, 12)
      });
    }
  }
...
export function NatalChartWheel({
  ...
  const placements = useMemo(
    () => computePlacements(dob, timeOfBirth),
    [dob, timeOfBirth]
  );
