...
import { TradeEmotionSelect, TradeTagSelect } from "@/components/trades/trade-tag-editor";
...
type TradeLogTableProps = {
  ...
  /** When true, setup / emotion columns use tag pickers from Settings. */
          ...
          {visible.map((t) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-2.5">
                  {editableTags ? (
                    <TradeTagSelect trade={t} />
                  ) : (
                    <span className="text-fg-muted">{t.setup_tag ?? "—"}</span>
                  )}
                </td>
                <td className="px-3 py-2.5">
                  {editableTags ? (
                    <TradeEmotionSelect trade={t} />
                  ) : (
                    <span className="text-fg-muted">{t.emotions?.[0] ?? "—"}</span>
                  )}
                </td>
              </tr>
            );
