...
-- ---------------------------- EA heartbeat -------------------------------
-- The EA fires a heartbeat on attach and on every poll cycle so the
-- Genesis setup wizard can flip to "Connected!" even before the user has
-- closed any trades. This RPC just upserts the per-account trade_files
-- row; no trade rows are touched.
